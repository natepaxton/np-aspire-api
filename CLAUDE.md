...
Backend for np-aspire: a single .NET 10 API behind an NGINX gateway, orchestrated locally with Aspire, runnable as containers via an Aspire-generated docker-compose file. The Angular frontend is a separate repo: https://github.com/natepaxton/np-aspire
...
- **Orchestration:** Aspire 13 AppHost (dev + source of truth); docker-compose generated by `aspire publish` into `deploy/compose/`
- **Gateway:** NGINX (`infra/nginx/`): `/` → frontend static-site container, `/api/` → API
