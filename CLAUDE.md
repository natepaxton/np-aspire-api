...
- Data Protection keys are in memory only (`AddInMemoryDataProtectionKeys`), since the API protects nothing. Anything that needs protected data (cookies, antiforgery, BFF) must first switch to persisted, encrypted keys (spec §3.1).
