...
- **Orchestration:** Aspire 13 AppHost — the only place topology is defined; containers/deployment files, when needed, come from Aspire publishers
...
- **Deferred:** NGINX gateway (until there are multiple APIs), containers/deployment, microservices split, RabbitMQ, BFF auth (see spec §8)
...
- The AppHost is the single source of truth for topology: add resources (database, cache, broker, services) there only. Don't add hand-written Dockerfiles, compose files, or NGINX config — containers/deployment files come from Aspire publishers when a target is chosen, and the gateway waits until there are multiple APIs.
